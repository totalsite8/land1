...
- `components/microSaas.ts` — единые данные каталога: 14 микроинструментов с подробностями (боль, признаки, что распутывает, кому подходит).
- `components/ProblemForm.tsx` + `app/api/lead/route.ts` — форма заявок с отправкой в Telegram через серверный route.
- `landing-journey.html` — универсальная пошаговая инструкция для создания лендинга в любой нише (не связана с этим продуктом).
