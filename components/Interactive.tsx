...
const scenes = [
  {
    ...
    more: "Клиент ждёт то, что наобещали голосом. Менеджер переспрашивает — и раздражает. Пока собирают концы по трём чатам, заявка остывает и уходит тому, кто ответил первым.",
    link: "#briefbox",
  ...
  },
  {
    ...
    more: "Коллеги отвечают на одни и те же вопросы вместо работы. Новичок стесняется спрашивать — и повторяет ошибки предшественника. Через месяц он знает ровно столько, сколько подслушал.",
    link: "#onboardone",
  ...
  },
  {
    ...
    more: "Утренняя смена узнаёт о конфликте от недовольного клиента — когда исправлять уже дороже. День начинается с расследования вчерашнего, а не с работы.",
    link: "#shifthandover",
...
const processSteps = [
  {
    ...
    text: "Не «нужна автоматизация», а «заявка теряется между Telegram и менеджером». Хватает пяти предложений и одного примера.",
    meta: "ОТ ВАС — 5 МИНУТ НА ФОРМУ"
  },
  {
    title: "Мы отвечаем в течение дня",
    text: "Уточняем, где процесс ломается, кто участвует и какой результат нужен. Вопросов много — короткий созвон без презентаций.",
    meta: "ОТ НАС — ВОПРОСЫ ПО СУТИ"
  },
  {
    title: "Собираем сценарий пилота",
    text: "Вы видите, что проверяем, какие данные нужны и что не входит в первую версию. Без «потом разберёмся».",
    meta: "ОТ НАС — ЧЕРНОВИК СЦЕНАРИЯ"
  },
  {
    title: "Берём приоритетные запросы в работу",
    text: "Сначала условия, потом сроки — без обещаний, которые нельзя выполнить. Узел не распутывается маленьким инструментом — скажем прямо.",
