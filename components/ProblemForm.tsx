...
export default function ProblemForm() {
  ...
  if (state === "sent") return <div className="formSuccess"><strong>Заявка у меня.</strong><br />Вернусь с вопросами в течение дня — отвечаю лично.</div>;
  return <form className="problemForm" onSubmit={submit}>
    ...
    <div className="formGrid">
      ...
      <label>Куда ответить<input required name="contact" placeholder="Телефон или Telegram" /></label>
      <label className="wide">Как вы ведёте это сейчас?<select required name="current" defaultValue=""><option value="" disabled>Выберите текущий способ</option><option>Таблицы (Excel, Google Sheets)</option><option>Чаты и мессенджеры</option><option>В памяти и на словах</option><option>CRM или 1С</option><option>Бумага и личный учёт</option><option>Всё сразу, у всех по-разному</option></select></label>
    </div>
    ...
    <button className="button buttonDark" disabled={state === "sending"} type="submit">{state === "sending" ? "Отправляю…" : "Отправить на разбор"} <span>↗</span></button>
    {state === "error" && <p className="formError">{error} <a href="https://t.me/bloodaman" target="_blank" rel="noreferrer">Написать мне в Telegram →</a></p>}
    <p className="formNote">Пароли, оплаты и клиентские базы присылать не нужно. Сначала просто разберём процесс.</p>
  </form>;
}
